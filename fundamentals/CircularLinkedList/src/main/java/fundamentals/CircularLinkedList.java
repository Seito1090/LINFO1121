...
public class CircularLinkedList<Item> implements Iterable<Item> {
    ...
    public Item remove(int index) {
        ...
        if (index >= this.n || index < 0) throw new IndexOutOfBoundsException();
        ...
        if (this.n == 1){
            ...
            for (int a = 0; a < index; a++){ // Get to the one right before what we need to delete
                currentNode = currentNode.next;
            }

            // delete the next in line 
            currentNode.next = currentNode.next.next;
            returnItem = currentNode.next.item;
        }
        this.nOp++;
        this.n--;
    ...
    private class ListIterator implements Iterator<Item> {
        ...
        @Override
        public Item next() {
            if (this.nOpS != nOp()) throw new ConcurrentModificationException();
