...
public class BinaryHeap {
    ...
    public void push(int value) {
        if (this.content.length <= this.size+1) this.increaseSize();
        this.content[this.size+1] = value;
        this.size++;
        //swim up
        int position = this.size;
        while (position > 1 && this.content[position/2] > this.content[position]){
            int swap = this.content[position];
            this.content[position] = this.content[position/2];
            this.content[position/2] = swap;
            position /=2 ;
        }
