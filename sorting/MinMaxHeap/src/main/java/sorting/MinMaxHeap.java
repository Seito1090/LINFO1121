...
public class MinMaxHeap<Key extends Comparable<Key>> {
     ...
     */
    public Key max() {
        if (this.size <= 1) return this.min();
        if (this.size == 2) return this.content[2];
        // if the size is bigger than 2, the max is either the on index 2 or 3
        Key k1 = this.content[2];
        Key k2 = this.content[3];
        if (this.higherThan(k1, k2)) return k1;
        return k2;
    }
    ...
    public void swim(int position) {
        //if we're at the beginning why even bother ? 
        if (position == 1) return; 
        Key currentKey = this.content[position];
        Key higherKey = this.content[position/2];
        Key higherHigherKey = (position/4 >= 0 && position/2 != position/4) ? this.content[position/4] : null;        
        int depth = getNodeDepth(position);
        boolean isMin = depth % 2 == 0;
        if (isMin) {
            //we are at a min layer, meaning the upper node is a max layer SO, we cannot be lower than it is!
            if (this.higherThan(currentKey, higherKey)){
                this.swap(position, position/2);
                //continue to swim up !
                this.swim(position/2);
            }
            //That said, we can be correctly positionned in terms of the parent (being lower than it) but still miss the mark in the general structure of the heap
            //Because what can happen is this : the node even higher is a min layer ! and if we are lower than that we need to swap too 
            else if (higherHigherKey != null && this.lessThan(currentKey, higherHigherKey)){
                this.swap(position, position/4);
                this.swim(position/4);
            }
        } else {
            //the opposite
            if (this.lessThan(currentKey, higherKey)){
                this.swap(position, position/2);
                //continue to swim up !
                this.swim(position/2);
            }
            else if (higherHigherKey != null && this.higherThan(currentKey, higherHigherKey)){
                this.swap(position, position/4);
                this.swim(position/4);
            }
        }
