...
public class MinPQLinked<Key> {
     ...
     */
    public Key delMin() {
        //Idea here : delete the min which is the root, take an item from the bottom and then sink it to restore the priority in the heap 
        if (this.size() == 0) throw new NoSuchElementException();
        Key min = this.root.value;
        if (this.size() == 1) {this.root = null; return min;}
        //Yank a big key from the bottom
        Key yanked = yankaNode();
        //Plop it at the top, the new node is just a blank one with a value set
        this.root.value = yanked;
        //sink it to restore the heap whatever
        sink(this.root);
        return min;
    }
    ...
    public void sink(Node n){
        ...
        while (n.left != null){
            ...
            Node toSwap;
            if (hasRight) toSwap = (greater(n.left, n.right)) ?  n.right : n.left;
            else toSwap = n.left;
            if (!greater(n, toSwap)) break;
            exch(n,toSwap);
            n = toSwap;
        } 
    }

    public Key yankaNode(){
        Node yanked = this.root;
        yanked.size--;
        while (yanked.left != null){
            boolean leftComplete = isPowerOfTwo(yanked.left.size+1);
            if (!leftComplete || yanked.right == null) yanked = yanked.left;
            else {
                //Check if right side is not complete
                if (!isPowerOfTwo(yanked.right.size+1)) yanked = yanked.right;
                else yanked = (yanked.left.size > yanked.right.size) ? yanked.left : yanked.right;
            }
            yanked.size--;
        }
        assert yanked.right == null;
        if (yanked.parent.left == yanked) yanked.parent.left = null;
        else yanked.parent.right = null;
