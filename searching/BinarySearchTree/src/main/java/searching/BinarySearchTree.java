...
public class BinarySearchTree {
    ...
    public static Integer ceil(BSTNode<Integer> root, int value) {
        if (root.getKey().equals(value)) return value;
        /* Just go down */
        BSTNode<Integer> currentNode = root;
        Integer returnValue = null;
        while (!currentNode.getKey().equals(value)){
            /* 2 cases, well 3, 1st the key is indeed one of the nodes then we can just 
            * itterate down and find it by going to the left / right according to the 
            * value, 2 The value isn't directly stored as one node so we have to return 
            * the parent that has the cutoff, 3 the value isn't there so we should return 
            * null, happens when the lowest value isn't low enough or the highest isn't high 
            * enough
            */
            if (currentNode.getKey().equals(value)) return value;
            else if (value < currentNode.getKey() && currentNode.getLeft() != null){
                returnValue = currentNode.key;
                currentNode = currentNode.getLeft(); 
            } else if (value > currentNode.getKey() && currentNode.getRight() != null) currentNode = currentNode.getRight();
            else break;
        }

        if (currentNode.getKey() < value) return returnValue;
