 ...
 */
class BirthdayMap {
    private TreeMap<String, ArrayList<Person>> theMap;
    ...
    void addPerson(Person person) {
        this.theMap.putIfAbsent(person.birthday, new ArrayList<>());
        this.theMap.get(person.birthday).add(person);
    }
    ...
    List<Person> getPeopleBornOnDate(String date) {
        return this.theMap.getOrDefault(date, new ArrayList<>());
    }
    ...
    List<Person> getPeopleBornInYear(String year) {
        ...
        String nextTime = (Integer.parseInt(year) + 1) + "-01-01";
        SortedMap<String, ArrayList<Person>> subMap = this.theMap.subMap(time, nextTime);
        for (List<Person> guy : subMap.values()) ppl.addAll(guy);
        return ppl;
    }
