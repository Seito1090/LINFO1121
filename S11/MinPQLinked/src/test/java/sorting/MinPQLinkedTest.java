...
public class MinPQLinkedTest {
    ...
    public void sinkTest() {
        ...
        //Section to print out the tree
        System.out.println("root: " + pq.root.value);
        System.out.println("children: " + pq.root.left.value + " " + pq.root.right.value);
        System.out.println("children on left side: " + pq.root.left.left.value + " " + pq.root.left.right.value);
        System.out.println("children on right side: " + pq.root.right.left.value + " " + pq.root.right.right.value);
        System.out.println("children on left side: " + pq.root.left.left.left.value + " " + pq.root.left.left.right.value);
        System.out.println("children on right side: " + pq.root.left.right.left.value + " " + pq.root.left.right.right.value);


        assertEquals(12, (int) pq.size());
        assertTrue(isMinHeapOrdered(pq.root));
        assertTrue(isEssentiallyComplete(pq.root));

        MinPQLinked<Integer>.Node n = pq.findLast();
        assertTrue(n.value >= 7 && n.value <= 9);

        //swap 9 with 3
        pq.exchange(n, pq.root);
        assertFalse(isMinHeapOrdered(pq.root));
        assertTrue(pq.root.value >= 7 && pq.root.value <= 9);
        assertEquals(12, (int) pq.size());
        assertEquals(n.value, 3);

        //Now sink the root value
        System.out.println("root: " + pq.root.value);
        System.out.println("children: " + pq.root.left.value + " " + pq.root.right.value);
        pq.singleSink(pq.root);
        assertEquals(4, pq.root.value);
