...
public class Monkeys {
    ...
    public static long evaluateRoot(List<Monkey> input) {
        /*Idea : put the values of yelling monkeys in the hash table, then use the values to compute the result of the operation monkeys*
        * */
        Hashtable<String, Node> hashTable = new Hashtable<>();
        for (Monkey monkey : input){
            if (monkey instanceof YellingMonkey){
                YellingMonkey yellingMonkey = (YellingMonkey) monkey;

                if (hashTable.containsKey(yellingMonkey.name)){
                    Node node = hashTable.get(yellingMonkey.name);
                    node.result = yellingMonkey.number;
                } else {
                    hashTable.put(yellingMonkey.name, new Node(yellingMonkey.name, yellingMonkey.number));
                }
            }
    ...
    private static class Node{
        String name;
        int result;
        public Node(String name, int result){
            this.name = name;
            this.result = result;
        }
    }
