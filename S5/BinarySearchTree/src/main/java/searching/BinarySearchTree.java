...
public class BinarySearchTree {
    ...
    public static Integer ceil(BSTNode<Integer> root, int value) {
        //initial case, if what we look for is somehow the first node.
        if (root.getKey() == value){
            return root.getKey();
        }

        //if it is not, we need to check if the value is bigger or smaller than the root.
        if (value < root.getKey()){ //smaller -> left if possible
            if (root.getLeft() != null){ //check if node exists on left
                Integer left = ceil(root.getLeft(), value);
                if (left != null){
                    return left;
                } else {
                    return root.getKey();
        ...
        if (value > root.getKey()){//bigger -> right if possible
            if (root.getRight() != null){ //check if node exists on right
